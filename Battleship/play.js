...
function placeShipsOnBoard(clickedCell, len, shipId, color) {
  ...
  if (ifCellInhabit(len, shipId, clickedCell) === true) {
    shipsOnBoard++;
    document.querySelectorAll(shipId)[0].remove();
    if (direction === "vertical") {
      placeVerticaly(clickedCell, len, color);
    } else {
      placeHorizontaly(clickedCell, len, color);
    }
  }
}

function placeVerticaly(clickedCell, len, color) {
...
function placeHorizontaly(clickedCell, len, color) {
